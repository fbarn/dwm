...
focusstack(const Arg *arg)
{
    ...
    Client *c;
    int start, num, pos, j, a = arg->i;
    getPos(&start, &num, &pos, ((selmon->lt[selmon->sellt]->arrange == deck) ? selmon->nmaster : -1));
    if (selmon->lt[selmon->sellt]->arrange == grid){
        int h;
        for (h=0; h*h<num; h++);
        a = a % 2 ?
            (pos % h != (h - (a>0)) % h  && pos + a < num ? a : 0) :
            (pos + h*(a/2) < num && pos + h*(a/2) >= 0 ? h*(a/2) : 0);
    }
    for (c = nexttiled(selmon->clients), j=-start; j < ((pos + a - start + num) % (num)); j++, c = nexttiled(c->next));
    if (c) focus(c);
    restack(selmon);
}
...
view(const Arg *arg)
{
	...
	arrange(selmon);
    if (previouslayout>=0){
        setlayout(&((Arg){ .v = &layouts[previouslayout] }));
        previouslayout=-1;
    }
    if (selmon->tagset[selmon->seltags]==511){
        Layout *l;
        int i=0;
        for(l = (Layout *)layouts; l != selmon->lt[selmon->sellt]; i++, l++);
        previouslayout=i;
